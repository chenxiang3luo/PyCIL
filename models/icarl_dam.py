...
import numpy as np
import torch
from dd_algorithms.dam import DataDAM
...
class iCaRL_DataDAM(BaseLearner):
    ...
    def _construct_exemplar_synthetic(self, data_manager, m, num_selection:int):
        ...
        syn_data, syn_lablel = self.dd.gen_synthetic_data(
            m, self._old_network, self.models, real_data, real_label, classes_range, self.path, use_trajectory=self.use_trajectory)
        if num_selection > 0:
