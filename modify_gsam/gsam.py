...
class GSAM(torch.optim.Optimizer):
    def __init__(self, params, base_optimizer, models:list, gsam_alpha, rho, adaptive=False, perturb_eps=1e-12, grad_reduce='mean', **kwargs):
    ...
    def maybe_no_sync(self):
        if torch.distributed.is_initialized():
            return [model.no_sync() for model in self.models_list]
        else:
    ...
    # @torch.no_grad()
    # def set_closure(self, forward_backward_fn, inputs, labels, **kwargs):
    ...
    #     def get_grad():
    #         self.zero_grad()
    #         # the forward_fn should include the backward function to get the gradient
    #         with torch.enable_grad():
    #             for _input, _label in zip(self.input_list, self.label_list):
    #                 forward_backward_fn(_input, _label, **kwargs)
    ...
    #     # need get the gradient by backward function in forward_backward_fn.
    #     if closure:
    #         get_grad = closure
    #     else:
    ...
    #         disable_running_stats(self.models_list)
    ...
    #     enable_running_stats(self.models_list)
    ...
    def set_closure(self, loss_fn, inputs, targets, **kwargs):
        ...
        def get_grad():
            self.base_optimizer.zero_grad()
            with torch.enable_grad():
                loss_list = loss_fn(inputs, targets, **kwargs)
            loss =loss_list
            loss.backward()
            return loss
    ...
    @torch.no_grad()
    def calculate_grad(self, closure=None):
