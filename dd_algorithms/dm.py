...
class DistributionMatching():
    ...
    def gen_synthetic_data(self,old_model,initial_data,real_data,real_label,classes):
        dsa_param = ParamDiffAug()
        images_all = real_data
        labels_all = real_label
        indices_class = [[] for c in range(classes)]
        ...
        image_syn = torch.randn(size=(classes*ipc, channel, im_size[0], im_size[1]), dtype=torch.float, requires_grad=True, device=self._device)
        label_syn = torch.tensor([np.ones(ipc)*i for i in range(classes)], dtype=torch.long, requires_grad=False, device=self._device).view(-1)
        ...
        for it in tqdm(range(Iteration+1)):
            ...
            if not BN: # for ConvNet
                ...
                loss += torch.sum((torch.mean(output_real.reshape(classes, batch_real, -1), dim=1) - torch.mean(output_syn.reshape(classes, ipc, -1), dim=1))**2)
